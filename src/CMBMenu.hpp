...
public:
  ...
  void printMenu() {
    ...
    for (int i = 0; i < m_numUsedElements; ++i) {
      ...
      DEBUG_PRINT(info_s);
      DEBUG_PRINT(F(", "));
      DEBUG_PRINTLN(m_nodes[i].getFID());
    }
  }
  ...
  void left() {
    if (m_index > 0) {
      ...
      int ctr = m_index - 1;
      int leftNodeIdx = -1;
      while ((-1 == leftNodeIdx) && (ctr >= 0) && (m_nodes[ctr].getLayer() >= m_nodes[m_index].getLayer())) {
  ...
  void right() {
    int rightNodeIdx = -1;
    int ctr = m_index + 1;
    while ((-1 == rightNodeIdx) && (ctr < m_numUsedElements) && (m_nodes[ctr].getLayer() >= m_nodes[m_index].getLayer())) {
      if (m_nodes[ctr].getLayer() == m_nodes[m_index].getLayer()) {
        rightNodeIdx = ctr;
      }
      ++ctr;
  ...
  int getCurrentLayer() {
    return m_nodes[m_index].getLayer();
  }

  //==========================================================================================
  //  Reset menu index to top
  //==========================================================================================
  void resetMenu() {
    m_index = 0;
  }
